...
router.post('/blogs', upload.single('blog_image'), async function (req, res, next) {
    ...
    const pinned = req.body.pinned | 0;
...
router.get("/blogs/:id",async function (req, res, next) {
  ...
  const promise2 = await pool.query("SELECT * FROM comments c LEFT OUTER JOIN images i on c.id = i.comment_id WHERE c.blog_id = ?;", [
    req.params.id,
  ]);
